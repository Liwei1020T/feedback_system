...
- **Authentication** (`/api/auth`) – login, refresh, logout, verify routes with in-memory token store.
- **Complaints** (`/api/complaints`) — CRUD operations, filtering, auto AI classification, plus `/api/complaints/:id/assist` for action recommendations and reply drafting. Supports pagination via `page`, `page_size`, `sort` (`created_at|priority|status`), `order` (`asc|desc`) and filter params (`kind`, `status`, `priority`, `plant`, `from_date`, `to_date`).
- **Replies** (`/api/replies`) – admin responses with optional email delivery via the pluggable `EmailService`.
- **Analytics** (`/api/analytics`) – dashboard metrics, trend data, and executive summaries (Claude-style heuristics).
- **Reports** (`/api/reports`) – generate/download summary reports for weekly/monthly/yearly periods.
- **Files** (`/api/upload`, `/api/files/:id`) – attachment upload/download/delete with server-side validation.
- **Department Analytics** (`/api/analytics/department-stats`) – per-category rollups showing totals, SLA breach rate, reopen rate, and backlog age.
- **Weekly Reports** (`/api/reports/weekly`) – list and view HTML/text weekly summaries generated by the background scheduler.
- **Feedback Presets** (`/api/profile/presets/feedback`) – store per-user filter presets for the All Feedback dashboard.
- **Email Templates** (`templates/email/`) – MJML-lite + Jinja templates rendered at runtime for reply notifications and weekly reports.
...
- Templates live under `templates/email/` – e.g., `reply.html`/`.txt` for feedback replies, `weekly_report.html`/`.txt` for scheduled reports.
