      ...
      <Card>
        <CardTitle>
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => refetch()}
              disabled={isFetching}
              className="text-xs font-semibold px-3 py-1.5 rounded-lg border border-red-200 text-red-700 bg-red-50 hover:bg-red-100 disabled:opacity-60"
            >
        ...
        <CardBody>
          {isLoading ? (
            ...
            <div className="text-center py-12 text-slate-500">No urgent tickets found</div>
          ) : (
            <div className="space-y-3">
              {items.map((c) => (
                <button
                  ...
                  className="w-full text-left flex items-center gap-4 p-4 bg-white hover:bg-red-50 border-2 border-slate-200 hover:border-red-300 rounded-xl transition-all group"
                >
                  {/* Indicator */}
